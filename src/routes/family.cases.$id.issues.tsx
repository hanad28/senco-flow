          ...
          {issue.status !== "dismissed" && (
            <div className="mt-4 space-y-2">
              <label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                Proposed amendment wording <AiLabel>AI suggestion</AiLabel>
              </label>
              <textarea
                value={draft}
                onChange={(e) => setDraft(e.target.value)}
                onBlur={() => onUpdate({ proposedAmendment: draft, status: issue.status === "not_started" && draft.trim() ? "drafted" : issue.status })}
                rows={3}
                className="w-full rounded-md border bg-surface px-3 py-2 text-sm resize-y"
                placeholder="Write or edit the amendment you would like included in your response…"
              />
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            {issue.status === "dismissed" ? (
              ...
              <>
                <button
                  type="button"
                  onClick={() => onUpdate({ proposedAmendment: draft, status: "ready" })}
                  disabled={!draft.trim()}
                ...
                <button type="button" onClick={() => onUpdate({ proposedAmendment: draft, status: "drafted" })} className="inline-flex items-center gap-1.5 min-h-11 px-3 rounded-md border text-sm hover:bg-accent">
                  Save as draft
                </button>
